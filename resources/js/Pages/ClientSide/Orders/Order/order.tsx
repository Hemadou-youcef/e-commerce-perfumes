...
const Order = ({ ...props }) => {
    ...
    const handleCancelOrder = () => {
        ...
        router.post(route('client_cancel_order', props?.order?.id), {}, {
            ...
            onSuccess: () => {
                toast({
                    title: t('custom.global.success'),
                    description: t('custom.order_page.order_cancelled'),
                    duration: 5000,
                })
            },
            onError: () => {
                toast({
                    ...
                    description: t('custom.order_page.order_cancelled_error'),
                    duration: 5000,
                })
            },
            onFinish: () => {
    ...
    return (
        <>
            <Head>
                <title>{t('custom.order_page.site_title')}</title>
                <meta name="description" content={t('custom.order_page.title')} />
                <meta name="keywords" content={t('custom.order_page.title')} />
            </Head>
            <div className="container mx-auto">
                ...
                <div className="flex flex-row justify-start items-center gap-2 my-5 ltr:font-sans rtl:font-arabic">
                    <Link href="/orders" className="text-sm font-bold text-gray-600 hover:text-gray-500 transition-colors  border-b pb-1">
                        {t('custom.order_page.return_to_orders')}
                    </Link>
                    <p className="text-sm font-medium text-gray-600 border-b pb-1">/</p>
                    <p className="text-sm font-medium text-gray-600 border-b pb-1">{t('custom.order_page.order')} #{props?.order?.id}</p>
                </div>
                {/* ORDER INFORMATION */}
                <div className="flex flex-col md:flex-row justify-between items-center  gap-5 mb-5 border-b pb-5 ltr:font-sans rtl:font-arabic">
                    <div className="flex flex-row justify-start items-center gap-2 ">
                        <p className="text-2xl font-bold text-gray-700">{t('custom.order_page.order')} #{props?.order?.id}</p>
                        ...
                        <AlertDialog>
                            <AlertDialogTrigger >
                                <Button variant="outline" className="text-sm font-medium text-red-500 hover:text-red-400 transition-colors gap-2 flex justify-center items-center">
                                    ...
                                    {t('custom.order_page.cancel_order')}
                                </Button>
                            ...
                            <AlertDialogContent className="font-arabic">
                                <AlertDialogHeader>
                                    ...
                                    <AlertDialogDescription>
                                        {t('custom.order_page.cancel_order_message')}
                                    </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                                    <AlertDialogCancel>
                ...
                <div className="flex flex-col justify-start items-start gap-5 mb-5 border-b pb-5 ltr:font-sans rtl:font-arabic">
                    ...
                    {/* DATE DE ORDER */}
                    <div className="flex flex-row justify-start items-center gap-2 ">
                        <p className="text-base font-bold text-gray-700">{t('custom.order_page.order_date')}:</p>
                        <p className="text-base font-medium text-gray-700"> {formatDate(props?.order?.created_at)}</p>
                    </div>
                    {/* SHIPPING AGENCY */}
                    <div className="flex flex-row justify-start items-center gap-2 ">
                        <p className="text-base font-bold text-gray-700">{t('custom.order_page.shipping_agency')}:</p>
                        <p className="text-base font-medium text-gray-700"> {currentLocale() === "fr" ? props?.order?.shipping_agency?.name : props?.order?.shipping_agency?.name_ar}</p>
                    </div>
                    {/* SHIPPING PRICE */}
                    <div className="flex flex-row justify-start items-center gap-2 ">
                        <p className="text-base font-bold text-gray-700">{t('custom.order_page.shipping_price')}:</p>
                        <p className="text-base font-medium text-gray-700"> {props?.order?.address?.shipping_price} {t("custom.global.da")} ({props?.order?.address?.shipping_fee?.agency_delivery_price == props?.order?.address?.shipping_price ? t("custom.order_page.agency_delivery") : t("custom.order_page.home_delivery")})</p>
                    </div>
                    {/* TOTAL PRICE */}
                    <div className="flex flex-row justify-start items-center gap-2 ">
                        <p className="text-base font-bold text-gray-700">{t('custom.order_page.total_price')}:</p>
                        <p className="text-base font-medium text-gray-700"> {props?.order?.total} {t('custom.global.da')}</p>
                    </div>
                    {/* ADDRESS */}
                    <div className="flex flex-row justify-start items-center gap-2 ">
                        <p className="text-base font-bold text-gray-700">{t('custom.order_page.shipping_address')}:</p>
                        <p className="text-base font-medium text-gray-700"> {props?.order?.address?.city} | {props?.order?.address?.street_address}</p>
                    </div>
                    {/* NUMBER OF PRODUCTS */}
                    <div className="flex flex-row justify-start items-center gap-2 ">
                        <p className="text-base font-bold text-gray-700">{t('custom.order_page.total_products')}:</p>
                    ...
                    <Table className="min-w-[700px] w-full ltr:font-sans rtl:font-arabic" dir={languageDir}>
                        <TableHeader>
                            <TableRow className="bg-gray-100 hover:bg-gray-100">
                                <TableHead className="rtl:text-right w-7">{t("custom.order_page.ID")}</TableHead>
                                <TableHead className="rtl:text-right">{t("custom.order_page.product")}</TableHead>
                                <TableHead className="rtl:text-right w-20">{t("custom.order_page.quantity")}</TableHead>
                                <TableHead className="rtl:text-right w-32">{t("custom.order_page.price")}</TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                            {props?.order?.order_products.map((product, index) => (
                                <TableRow key={index} className="hover:bg-gray-100">
                                    ...
                                    <TableCell className="font-bold text-xs ">{product?.quantity * product?.product_price?.quantity} {t('custom.units.' + product?.product_price?.unit?.toLowerCase())}</TableCell>
                                    <TableCell className="font-bold text-xs">{product?.price} {t('custom.global.da')}</TableCell>
                                </TableRow>
                            ))}
                            {/* <TableRow>
