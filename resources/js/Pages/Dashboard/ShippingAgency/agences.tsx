...
import { ShippingAgenciesInfo, columns } from "@/components/columns/shippingAgencies";
        ...
        <>
            <div className="flex flex-row justify-between items-center px-5 py-2 gap-2 h-14 w-full sticky top-0 bg-gray-50 shadow-sm z-10">
                ...
                <Link href="/dashboard/shipping_agencies/create">
                    <Button className="flex items-center p-0 px-5 h-9 md:h-10 md:rounded-md">
                        ...
                        <span className="ml-2 hidden md:block">Ajouter un agent</span>
                    </Button>
                </Link>
            </div>
            <div className="flex flex-col gap-2 mx-2 md:mx-10 py-2">
                ...
                <div className="max-w-full overflow-x-auto pb-2">
                    <DataTable columns={columns} data={data} baseUrl="/dashboard/shipping_agencies/" />
                </div>
                <Pagination meta={props?.shippingAgencies} />
            </div>
        </>
    );
}
...
Agences.layout = (page: React.ReactNode) => <DashboardMainLayout children={page} />;
